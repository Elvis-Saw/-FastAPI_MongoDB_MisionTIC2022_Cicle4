...
@app.post("/", response_description="Add new student", response_model=StudentModel) #Metodo POST
...
@app.get( "/", response_description="List all students", response_model=List[StudentModel] ) #metodo GET
async def list_students(): 
          students = await db["tripulantes"].find().to_list(1000) 
          return students

@app.put("/{id}", response_description="Update a student",response_model=StudentModel) #Metodo PUT
async def update_student(id: str, student: UpdateStudentModel = Body(...)):
           student = {k: v for k, v in student.dict().items() if v is not None} 
           if len(student) >= 1: 
                    update_result = await db["tripulantes"].update_one({"_id": id}, {"$set": student})       
                    if update_result.modified_count == 1: 
                              if ( updated_student := await db["tripulantes"].find_one({"_id": id})) is not None: 
                                        return updated_student
                    if (existing_student := await db["tripulantes"].find_one({"_id": id})) is not None:
                              return existing_student 
                              raise HTTPException(status_code=404, detail=f"Student {id} not found")          

@app.delete("/{id}", response_description="Delete a student") #Metodo DELETE
